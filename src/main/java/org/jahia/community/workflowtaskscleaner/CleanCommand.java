...
public class CleanCommand implements Action {
    ...
    private static final List<String> SQL_STMTS = Arrays.asList(
            "DELETE FROM jbpm_people_assignm_pot_owners WHERE task_id IN (SELECT task_id FROM jbpm_task WHERE status='Completed' OR status='Exited')",
            "DELETE FROM jbpm_people_assignments_bas WHERE task_id IN (SELECT task_id FROM jbpm_task WHERE status='Completed' OR status='Exited')",
            "DELETE FROM jbpm_i18ntext WHERE task_subjects_id IN (SELECT task_subjects_id FROM jbpm_task WHERE status='Completed' OR status='Exited')",
            "DELETE FROM jbpm_i18ntext WHERE task_names_id IN (SELECT task_names_id FROM jbpm_task WHERE status='Completed' OR status='Exited')",
            "DELETE FROM jbpm_i18ntext WHERE task_descriptions_id IN (SELECT task_descriptions_id FROM jbpm_task WHERE status='Completed' OR status='Exited')",
            "DELETE FROM jbpm_task WHERE status='Completed' OR status='Exited'"
    );
